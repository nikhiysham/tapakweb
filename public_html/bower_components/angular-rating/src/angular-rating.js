(function() {
  ...
  angular.module("angular-rating", []).component("rating", {
    template:
      '<span class="rating-container" ng-mouseleave="model.control_onMouseLeave()">' +
    ...
    controller: [
      ...
      function($timeout, $scope) {
        ...
        if (!model.size) model.size = "20px";

        if (!model.color) model.color = "#F7EB90";

        if (!model.highColor) model.highColor = "#F7EB90";
        ...
        model.onMouseOver = function(starIndex) {
          // console.log("mouseOver:", starIndex);
          if (isInteractive()) {
        ...
        var star = {
          selector: ".star",
          rules: ["font-size: 18px", "color: #ddd", "cursor: pointer"]
        };
        var starOthers = {
          selector: ".star+.star",
          rules: ["margin-left: 3px"]
        };
        var starOn = {
          selector: ".star.star-on",
          rules: ["color:" + model.color]
        };
        var starHigh = {
          selector: ".star.star-high",
          rules: ["color:" + model.highColor]
        };

        var ratingCSS = rating.selector + "{" + rating.rules.join(";") + "}";
        var starCSS = star.selector + "{" + star.rules.join(";") + "}";
        var starOthersCSS =
          starOthers.selector + "{" + starOthers.rules.join(";") + "}";
        var starOnCSS = starOn.selector + "{" + starOn.rules.join(";") + "}";
        var starHighCSS =
          starHigh.selector + "{" + starHigh.rules.join(";") + "}";
        angular
          .element(document)
          .find("head")
          .prepend(
